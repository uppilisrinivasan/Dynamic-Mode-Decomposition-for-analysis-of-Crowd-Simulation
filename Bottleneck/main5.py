...
for delay in range(1,11):
    ...
    mae_error_final[delay]= mae_error
    mse_error_final[delay] = mse_error
    rmse_error_final[delay]= rmse_error

plt.figure(figsize=(10, 6))
for key, array in mae_error_final.items():
    plt.plot(array, label=f'Array {key}')
...
# Add legend
plt.legend()
plt.savefig('plot_mae_delay_rank.png')

# Create a new figure
plt.figure(figsize=(10, 6))

# Create a list to store the arrays
arrays = [array for key, array in mae_error_final.items()]

# Create a box plot for the arrays
sns.boxplot(data=arrays)
...
plt.savefig('plot_mae_delay_rank_box.png')
